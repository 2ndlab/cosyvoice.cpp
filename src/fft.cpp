#include "fft.h"
#ifndef COSYVOICE_FFT_STANDALONE
#include "ggml-fft.h"
        ...
        if constexpr (C.avx)
        {
            if (fstride == 1)
                for (; i + 7 < m; i += 8)
                {
                    ...
                    __m256 tr = simd_fmsub_ps<C>(F2r, Twr, _mm256_mul_ps(F2i, Twi));
                    __m256 ti = simd_fmadd_ps<C>(F2r, Twi, _mm256_mul_ps(F2i, Twr));
            ...
            else for (; i + 7 < m; i += 8)
            {
                ...
                if constexpr (C.avx2)
                {
                    const __m256i vidx0 = _mm256_setr_epi32(
                ...
                {
                    Twr = simd_load8_strided_ps(tw_r + i * fstride, fstride);
                    Twi = simd_load8_strided_ps(tw_i + i * fstride, fstride);
                }
        ...
        if constexpr (C.sse42)
        {
            for (; i + 3 < m; i += 4)
            {
                ...
                else
                {
                    ...
                    {
                        Twr = simd_load4_strided_ps(tw_r + i * fstride, fstride);
                        Twi = simd_load4_strided_ps(tw_i + i * fstride, fstride);
                    }
                }

                __m128 tr = simd_fmsub_ps<C>(F2r, Twr, _mm_mul_ps(F2i, Twi));
                __m128 ti = simd_fmadd_ps<C>(F2r, Twi, _mm_mul_ps(F2i, Twr));
...
static void kf_bfly2(
    ...
    int          m)
{
    simd_dispatch<fft_bfly2_kernel>(Foutr, Fouti, fstride, tw_r, tw_i, m);
}
    ...
    static void run(float* Foutr, float* Fouti, int fstride, float* twr, float* twi, int m)
    {
            ...
            for (; k + 7 < m; k += 8)
            {
                ...
                if constexpr (C.avx2)
                {
                    const __m256i vidx0 = _mm256_setr_epi32(
                        ...
                        4 * fstride, 5 * fstride, 6 * fstride, 7 * fstride);
                    const __m256i vidx1 = _mm256_mullo_epi32(
                        vidx0, _mm256_set1_epi32(2));  /* 2 * fstride */
                    const __m256i vidx2 = _mm256_mullo_epi32(
                ...
                else
                {
                    ...
                    {
                        t1r = simd_load8_strided_ps(twr + k * fstride, fstride);
                        t1i = simd_load8_strided_ps(twi + k * fstride, fstride);
                    }

                    t2r = simd_load8_strided_ps(twr + 2 * k * fstride, 2 * fstride);
                    t2i = simd_load8_strided_ps(twi + 2 * k * fstride, 2 * fstride);

                    t3r = simd_load8_strided_ps(twr + 3 * k * fstride, 3 * fstride);
                    t3i = simd_load8_strided_ps(twi + 3 * k * fstride, 3 * fstride);
                }

                __m256 s0r = simd_fmsub_ps<C>(r1, t1r, _mm256_mul_ps(i1, t1i));
                __m256 s0i = simd_fmadd_ps<C>(r1, t1i, _mm256_mul_ps(i1, t1r));

                __m256 s1r = simd_fmsub_ps<C>(r2, t2r, _mm256_mul_ps(i2, t2i));
                __m256 s1i = simd_fmadd_ps<C>(r2, t2i, _mm256_mul_ps(i2, t2r));

                __m256 s2r = simd_fmsub_ps<C>(r3, t3r, _mm256_mul_ps(i3, t3i));
                __m256 s2i = simd_fmadd_ps<C>(r3, t3i, _mm256_mul_ps(i3, t3r));
        ...
        if constexpr (C.sse42)
        {
            for (; k + 3 < m; k += 4)
            {
                ...
                if constexpr (C.avx2)
                {
                    const __m128i vidx0 = _mm_setr_epi32(0, fstride, 2 * fstride, 3 * fstride);
                    const __m128i vidx1 = _mm_mullo_epi32(vidx0, _mm_set1_epi32(2));
                    const __m128i vidx2 = _mm_mullo_epi32(vidx0, _mm_set1_epi32(3));
                ...
                {
                    t1r = simd_load4_strided_ps(twr + k * fstride, fstride);
                    t1i = simd_load4_strided_ps(twi + k * fstride, fstride);

                    t2r = simd_load4_strided_ps(twr + 2 * k * fstride, 2 * fstride);
                    t2i = simd_load4_strided_ps(twi + 2 * k * fstride, 2 * fstride);

                    t3r = simd_load4_strided_ps(twr + 3 * k * fstride, 3 * fstride);
                    t3i = simd_load4_strided_ps(twi + 3 * k * fstride, 3 * fstride);
                }

                __m128 s0r = simd_fmsub_ps<C>(r1, t1r, _mm_mul_ps(i1, t1i));
                __m128 s0i = simd_fmadd_ps<C>(r1, t1i, _mm_mul_ps(i1, t1r));

                __m128 s1r = simd_fmsub_ps<C>(r2, t2r, _mm_mul_ps(i2, t2i));
                __m128 s1i = simd_fmadd_ps<C>(r2, t2i, _mm_mul_ps(i2, t2r));

                __m128 s2r = simd_fmsub_ps<C>(r3, t3r, _mm_mul_ps(i3, t3i));
                __m128 s2i = simd_fmadd_ps<C>(r3, t3i, _mm_mul_ps(i3, t3r));
...
static void kf_bfly4(
    ...
    int m)
{
    simd_dispatch<fft_bfly4_kernel>(Foutr, Fouti, fstride, twr, twi, m);
}
        ...
        if constexpr (C.avx)
        {
            for (const __m256 v_half = _mm256_set1_ps(0.5f), v_epi3 = _mm256_set1_ps(epi3_i); k + 7 < m; k += 8)
            {
                ...
                else
                {
                    ...
                    {
                        tw1r = simd_load8_strided_ps(tw1_r + k * fstride, fstride);
                        tw1i = simd_load8_strided_ps(tw1_i + k * fstride, fstride);
                    }
                    tw2r = simd_load8_strided_ps(tw1_r + 2 * k * fstride, 2 * fstride);
                    tw2i = simd_load8_strided_ps(tw1_i + 2 * k * fstride, 2 * fstride);
                }

                __m256 sr1 = simd_fmsub_ps<C>(fr1, tw1r, _mm256_mul_ps(fi1, tw1i));
                __m256 si1 = simd_fmadd_ps<C>(fr1, tw1i, _mm256_mul_ps(fi1, tw1r));
                __m256 sr2 = simd_fmsub_ps<C>(fr2, tw2r, _mm256_mul_ps(fi2, tw2i));
                __m256 si2 = simd_fmadd_ps<C>(fr2, tw2i, _mm256_mul_ps(fi2, tw2r));
        ...
        if constexpr (C.sse42)
        {
                ...
                k + 3 < m; k += 4)
            {
                ...
                if (fstride == 1)
                {
                    ...
                    if constexpr (C.avx2)
                    {
                        const __m128i idx2 = _mm_setr_epi32(k * 2, (k + 1) * 2, (k + 2) * 2, (k + 3) * 2);
                        tw2r = _mm_i32gather_ps(tw1_r, idx2, 4);
                        tw2i = _mm_i32gather_ps(tw1_i, idx2, 4);
                    }
                    else
                    {
                        tw2r = simd_load4_strided_ps(tw1_r + 2 * k, 2);
                        tw2i = simd_load4_strided_ps(tw1_i + 2 * k, 2);
                    }
                }
                else
                {
                    if constexpr (C.avx2)
                    {
                        const __m128i idx1 = _mm_setr_epi32(
                            k * fstride, (k + 1) * fstride, (k + 2) * fstride, (k + 3) * fstride);
                        const __m128i idx2 = _mm_mullo_epi32(idx1, _mm_set1_epi32(2));
                    ...
                    {
                        tw1r = simd_load4_strided_ps(tw1_r + k * fstride, fstride);
                        tw1i = simd_load4_strided_ps(tw1_i + k * fstride, fstride);
                        tw2r = simd_load4_strided_ps(tw1_r + 2 * k * fstride, 2 * fstride);
                        tw2i = simd_load4_strided_ps(tw1_i + 2 * k * fstride, 2 * fstride);
                    }
                }

                __m128 sr1 = simd_fmsub_ps<C>(fr1, tw1r, _mm_mul_ps(fi1, tw1i));
                __m128 si1 = simd_fmadd_ps<C>(fr1, tw1i, _mm_mul_ps(fi1, tw1r));
                __m128 sr2 = simd_fmsub_ps<C>(fr2, tw2r, _mm_mul_ps(fi2, tw2i));
                __m128 si2 = simd_fmadd_ps<C>(fr2, tw2i, _mm_mul_ps(fi2, tw2r));
...
static void kf_bfly3(
    ...
    int    m)
{
    simd_dispatch<fft_bfly3_kernel>(Foutr, Fouti, fstride, tw1_r, tw1_i, m);
}
    ...
    static void run(float* Foutr, float* Fouti, int fstride, const float* tw_r, const float* tw_i, int m)
    {
        ...
        if constexpr (C.avx)
        {
            for (const __m256 v_yar = _mm256_set1_ps(yar),
                ...
                __m256 tw1r, tw1i, tw2r, tw2i, tw3r, tw3i, tw4r, tw4i;
                if constexpr (C.avx2)
                {
                    __m256i idx1 = _mm256_setr_epi32(
                ...
                else
                {
                    ...
                    {
                        tw1r = simd_load8_strided_ps(tw_r + u * fstride, fstride);
                        tw1i = simd_load8_strided_ps(tw_i + u * fstride, fstride);
                    }
                    tw2r = simd_load8_strided_ps(tw_r + 2 * u * fstride, 2 * fstride);
                    tw2i = simd_load8_strided_ps(tw_i + 2 * u * fstride, 2 * fstride);
                    tw3r = simd_load8_strided_ps(tw_r + 3 * u * fstride, 3 * fstride);
                    tw3i = simd_load8_strided_ps(tw_i + 3 * u * fstride, 3 * fstride);
                    tw4r = simd_load8_strided_ps(tw_r + 4 * u * fstride, 4 * fstride);
                    tw4i = simd_load8_strided_ps(tw_i + 4 * u * fstride, 4 * fstride);
                }
                ...
                __m256 s5r = simd_fmadd_ps<C>(s7r, v_yar, f0r);
                s5r = simd_fmadd_ps<C>(s8r, v_ybr, s5r);
                __m256 s5i = simd_fmadd_ps<C>(s7i, v_yar, f0i);
                s5i = simd_fmadd_ps<C>(s8i, v_ybr, s5i);

                __m256 s6r = simd_fmadd_ps<C>(s10i, v_yai, _mm256_mul_ps(s9i, v_ybi));
                __m256 s6i = _mm256_sub_ps(_mm256_setzero_ps(), simd_fmadd_ps<C>(s10r, v_yai, _mm256_mul_ps(s9r, v_ybi)));
                ...
                __m256 s11r = simd_fmadd_ps<C>(s7r, v_ybr, f0r);
                s11r = simd_fmadd_ps<C>(s8r, v_yar, s11r);
                __m256 s11i = simd_fmadd_ps<C>(s7i, v_ybr, f0i);
                ...
                __m256 s12r = simd_fmsub_ps<C>(s9i, v_yai, _mm256_mul_ps(s10i, v_ybi));
                __m256 s12i = simd_fmsub_ps<C>(s10r, v_ybi, _mm256_mul_ps(s9r, v_yai));
        ...
        if constexpr (C.sse42)
        {
            ...
            {
                __m128 tw1r, tw1i, tw2r, tw2i, tw3r, tw3i, tw4r, tw4i;
                if constexpr (C.avx2)
                {
                ...
                {
                    tw1r = simd_load4_strided_ps(tw_r + u * fstride, fstride);
                    tw1i = simd_load4_strided_ps(tw_i + u * fstride, fstride);

                    tw2r = simd_load4_strided_ps(tw_r + 2 * u * fstride, 2 * fstride);
                    tw2i = simd_load4_strided_ps(tw_i + 2 * u * fstride, 2 * fstride);

                    tw3r = simd_load4_strided_ps(tw_r + 3 * u * fstride, 3 * fstride);
                    tw3i = simd_load4_strided_ps(tw_i + 3 * u * fstride, 3 * fstride);

                    tw4r = simd_load4_strided_ps(tw_r + 4 * u * fstride, 4 * fstride);
                    tw4i = simd_load4_strided_ps(tw_i + 4 * u * fstride, 4 * fstride);
                }
                ...
                __m128 s5r = simd_fmadd_ps<C>(s7r, v_yar, f0r);
                s5r = simd_fmadd_ps<C>(s8r, v_ybr, s5r);
                __m128 s5i = simd_fmadd_ps<C>(s7i, v_yar, f0i);
                s5i = simd_fmadd_ps<C>(s8i, v_ybr, s5i);

                __m128 s6r = simd_fmadd_ps<C>(s10i, v_yai, _mm_mul_ps(s9i, v_ybi));
                __m128 s6i = _mm_sub_ps(_mm_setzero_ps(), simd_fmadd_ps<C>(s10r, v_yai, _mm_mul_ps(s9r, v_ybi)));
                ...
                __m128 s11r = simd_fmadd_ps<C>(s7r, v_ybr, f0r);
                s11r = simd_fmadd_ps<C>(s8r, v_yar, s11r);
                __m128 s11i = simd_fmadd_ps<C>(s7i, v_ybr, f0i);
                s11i = simd_fmadd_ps<C>(s8i, v_yar, s11i);
                ...
                __m128 s12r = simd_fmsub_ps<C>(s9i, v_yai, _mm_mul_ps(s10i, v_ybi));
                __m128 s12i = simd_fmsub_ps<C>(s10r, v_ybi, _mm_mul_ps(s9r, v_yai));
...
static void kf_bfly5(
    ...
    int m)
{
    simd_dispatch<fft_bfly5_kernel>(Foutr, Fouti, fstride, tw_r, tw_i, m);
}
            ...
            for (int q1 = 0; q1 < p; ++q1) {
                ...
                if constexpr (C.avx)
                {
                    for (; q + 7 < p; q += 8) {
                        ...
                        {
                            twr = simd_load8_indexed_ps(twiddles_r, idx);
                            twi = simd_load8_indexed_ps(twiddles_i, idx);
                        }

                        __m256 sr = _mm256_loadu_ps(scratchr + q);
                        __m256 si = _mm256_loadu_ps(scratchi + q);

                        __m256 tr = simd_fmsub_ps<C>(sr, twr, _mm256_mul_ps(si, twi));
                        __m256 ti = simd_fmadd_ps<C>(sr, twi, _mm256_mul_ps(si, twr));
                ...
                if constexpr (C.sse42)
                {
                    for (; q + 3 < p; q += 4)
                    {
                        ...
                        {
                            twr = simd_load4_indexed_ps(twiddles_r, idx);
                            twi = simd_load4_indexed_ps(twiddles_i, idx);
                        }

                        __m128 sr = _mm_loadu_ps(scratchr + q);
                        __m128 si = _mm_loadu_ps(scratchi + q);

                        __m128 tr = simd_fmsub_ps<C>(sr, twr, _mm_mul_ps(si, twi));
                        __m128 ti = simd_fmadd_ps<C>(sr, twi, _mm_mul_ps(si, twr));
...
static void kf_bfly_generic(
    ...
    float* scratchi)
{
    simd_dispatch<fft_bfly_generic_kernel>(Foutr, Fouti, fstride, twiddles_r, twiddles_i, Norig, m, p, scratchr, scratchi);
}
    ...
    static void run(float* twiddles_r, float* twiddles_i, int n, float factor)
    {
        ...
        if constexpr (C.avx)
        {
            ...
            for (; i + 7 < n; i += 8)
            {
                ...
                _mm256_storeu_ps(twiddles_r + i, simd_cos_ps(phase));
                _mm256_storeu_ps(twiddles_i + i, simd_sin_ps(phase));
            }
        }
        if constexpr (C.sse42)
        {
            ...
            __m128 base128 = _mm_setr_ps(static_cast<float>(i), static_cast<float>(i) + 1, static_cast<float>(i) + 2, static_cast<float>(i) + 3);
            for (; i + 3 < n; i += 4)
            {
                __m128 phase = _mm_mul_ps(base128, factor128);
                base128 = _mm_add_ps(base128, _4);
                _mm_storeu_ps(twiddles_r + i, simd_cos_ps(phase));
                _mm_storeu_ps(twiddles_i + i, simd_sin_ps(phase));
            }
        }
        for (; i < n; ++i)
        {
...
struct safe_divide_kernel
{
    static void run(float* data, const float* denom, size_t n, float min_denom)
    {
        ...
        if constexpr (C.avx)
        {
            const __m256 eps_v = _mm256_set1_ps(min_denom);
            for (; i + 7 < n; i += 8)
            {
                __m256 d = _mm256_loadu_ps(denom + i);
                __m256 y = _mm256_loadu_ps(data + i);
        ...
        if constexpr (C.sse42)
        {
            const __m128 eps_128 = _mm_set_ps1(min_denom);
            for (; i + 3 < n; i += 4)
            {
                __m128 d = _mm_loadu_ps(denom + i);
                __m128 y = _mm_loadu_ps(data + i);
